...
Output sanitizer полностью убран из пути ответа LLM — ни клиентский ответ, ни audit body logging больше не получают санитайзированный вывод.

Ранее (v3.1) output sanitizer был убран из «критического пути» к клиенту, но для audit body logging (`log_body: true`) всё равно запускался постфактум на полном тексте в блоке `finally`. Теперь он не применяется нигде к ответу LLM.

### Обоснование

Output sanitizer на ответе имеет смысл только если **gateway знает больше, чем клиент** — например, RAG, где gateway инжектирует данные других пользователей. В нашем сценарии клиент уже знает всё, что знает gateway. Санитайзировать ответ для того же клиента, который его запросил, бессмысленно.

Для audit body logging польза тоже неочевидна: если в ответе LLM есть PII, оно попало туда из запроса клиента — который уже санитайзирован на входе. Audit хранит placeholder-версию входных сообщений, чего достаточно.

Поле `output_chain` в `ChatService` и конфиг-секция `sanitizers.output` остаются в коде — инфраструктура готова к подключению, если появится сценарий, где gateway инжектирует данные из внешних источников.

### Изменения в коде

- `ChatService.complete()`: убран блок применения `self._output.run()` к `response.content`.
- `ChatService.complete_stream()`: убраны флаг `has_output`, буферизация чанков для output sanitizer, постфактум-запуск в `finally`.
- `output_actions` остаётся пустым `[]` во всех audit-записях.

---

## v3.4 — Tools Passthrough + PII Restoration in Tool Calls (2026-06-12)

**No breaking changes.** Клиенты теперь могут передавать `tools` в любом запросе; gateway прозрачно проксирует их к upstream LLM и возвращает `tool_calls` клиенту. Клиент владеет агентным циклом.

### Почему логика инструментов на стороне клиента

Альтернатива — реестр инструментов на стороне gateway: gateway знает о Sentry MCP, внутренних API и т.д., инжектирует нужные схемы по идентификатору клиента. Отклонено по той же причине, по которой мы отклонили хардкод адаптеров: каждый новый инструмент требовал бы деплоя gateway, а схемы инструментов со временем расходятся с реальными реализациями. Gateway — прокси, а не сервисная сетка. Клиенты отправляют то, что им нужно.

### Восстановление PII в аргументах tool_call

Input sanitizer заменяет `john@example.com` на `[EMAIL_ADDRESS_3f2a1b0c]` до того, как LLM его увидит. LLM может вернуть этот placeholder в аргументе вызова инструмента `send_email(to="[EMAIL_ADDRESS_3f2a1b0c]")`. Без восстановления клиент получает placeholder — вызов инструмента сломан.

**Фикс для не-стриминга:** сериализуем `response.tool_calls` в JSON, запускаем `context.restore()`, парсим обратно. Placeholder — непрерывная строка внутри сериализованного JSON, простая замена строк работает.

**Фикс для стриминга:** аргументы tool_call приходят посимвольно в множестве SSE-чанков. `[EMAIL_ADDRESS_3f2a1b0c]` может прийти как `[`, `EMAIL`, `_ADDRESS_3`, `f2a1b0c]` — четыре отдельных события. Побайтовый `str.replace()` его не найдёт.

Решение: отдельный `StreamingRestorer` на каждый `tool_call index`. Та же логика look-ahead буфера, что используется для текстового контента (v3.3), применяется к потоку аргументов каждого инструмента. Символы до `[` пересылаются немедленно. Буфер удерживает только байты, которые могут быть началом placeholder-а. Когда полный placeholder найден, оригинальное значение отдаётся одним чанком. Задержка ограничена длиной placeholder-а (~26 символов).

Это даёт настоящий стриминг аргументов — фрагменты появляются у клиента в реальном времени, задержка только вокруг самого placeholder-а. Вариант с буферизацией всех аргументных дельт и отдачей одного чанка в конце был реализован первым, но откатан: проще, но ломал стриминг для tool_calls при наличии PII.

### Поведение `[` и `]` в обычном тексте

`_probe()` restorer-а буферизует только когда буфер является префиксом зарегистрированного placeholder-а. `[category]` вызывает задержку в один чанк на `[`; как только приходит `c`, проверка префикса не проходит и `[` отдаётся немедленно. Контент не теряется. Ложное срабатывание (LLM генерирует строку, идентичную зарегистрированному placeholder-у) требует угадать 4-байтовый случайный hex-суффикс — 2³² вариантов, пренебрежимо мало на практике.

### finish_reason в стриминге

Upstream отправляет `finish_reason: "stop"` в финальном SSE-чанке независимо от того, были ли вызваны инструменты. Доверять ему нельзя. Фикс: отслеживаем `has_tool_calls` во время стрима, переопределяем `finish_reason` на `"tool_calls"` если была получена хоть одна tool_call дельта.
